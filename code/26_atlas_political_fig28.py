...
print("--- Generating Atlas-Quality Political Country Map (Figure 28 Panel B) ---")
...
# Distinct Country Color Palette (Atlas Style)
country_fill_colors = {
    'GBR': '#e9d5ff', 'NOR': '#ccfbf1', 'SWE': '#dcfce7', 'FIN': '#e0f2fe',
    'FRA': '#fef9c3', 'ESP': '#ffedd5', 'DEU': '#fce7f3', 'ITA': '#fef3c7',
    'POL': '#f1f5f9', 'UKR': '#fef08a', 'TUR': '#fae8ff', 'GEO': '#dcfce7',
    'ARM': '#ffedd5', 'AZE': '#fee2e2', 'IRN': '#ecfccb', 'IRQ': '#fef3c7',
    'SYR': '#f3e8ff', 'PRT': '#ffedd5', 'IRL': '#dcfce7', 'GRC': '#e0f2fe',
    'ROU': '#fce7f3', 'BGR': '#fef9c3', 'KAZ': '#f1f5f9', 'RUS': '#f8fafc'
}

# Load Natural Earth Admin-0 Countries Shapefile
shpfilename = shpreader.natural_earth(resolution='50m', category='cultural', name='admin_0_countries')
reader = shpreader.Reader(shpfilename)
country_records = list(reader.records())
...
# ====================================================
# PANEL B: Atlas Political Country Map (Natural Earth 50m Polygons)
# ====================================================
ax2.add_feature(cfeature.OCEAN.with_scale('50m'), facecolor='#bae6fd', zorder=1) # Rich ocean blue

# Render Country Geometries with Distinct Pastel Atlas Fills
for country in country_records:
    iso_a3 = country.attributes.get('ADM0_A3', '')
    fill_color = country_fill_colors.get(iso_a3, '#f1f5f9')
    
    ax2.add_geometries(
        [country.geometry],
        crs=proj,
        facecolor=fill_color,
        edgecolor='#334155', # Dark slate border
        linewidth=0.9,
        zorder=2
    )

# Render Inland Lakes and Rivers
ax2.add_feature(cfeature.LAKES.with_scale('50m'), facecolor='#bae6fd', edgecolor='#38bdf8', linewidth=0.6, zorder=3)
ax2.add_feature(cfeature.RIVERS.with_scale('50m'), edgecolor='#38bdf8', linewidth=0.7, zorder=3)

# Major Country Centroid Labels
country_annotations = [
...
for cname, cx, cy in country_annotations:
    ax2.text(cx, cy, cname, fontsize=8.5, fontweight='bold', color='#1e293b', alpha=0.85,
             ha='center', va='center', transform=proj,
             bbox=dict(boxstyle='square,pad=0.2', facecolor='#ffffff', alpha=0.8, edgecolor='none'), zorder=5)
...
ax2.set_title('B: Atlas Political Country Map (Natural Earth 50m Sovereign Geometries)', fontsize=13, fontweight='bold')

# Gridlines for both Cartopy axes
for ax in [ax1, ax2]:
    gl = ax.gridlines(draw_labels=True, linestyle=':', alpha=0.5, color='#64748b')
    gl.top_labels = False
...
legend_elements = [
    ...
    plt.Line2D([0], [0], color='#334155', lw=1.2, label='Natural Earth 50m Sovereign Borders')
]
...
print("Successfully generated atlas-quality political country map for Figure 28 Panel B!")
